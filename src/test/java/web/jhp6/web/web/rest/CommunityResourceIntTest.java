package web.jhp6.web.web.rest;

import web.jhp6.web.JhipsterPress06App;
...
import web.jhp6.web.domain.Calbum;
import web.jhp6.web.domain.Interest;
import web.jhp6.web.domain.Activity;
import web.jhp6.web.domain.Celeb;
import web.jhp6.web.repository.CommunityRepository;
import web.jhp6.web.service.CommunityService;
import web.jhp6.web.service.dto.CommunityDTO;
import web.jhp6.web.service.mapper.CommunityMapper;
import web.jhp6.web.web.rest.errors.ExceptionTranslator;
import web.jhp6.web.service.dto.CommunityCriteria;
import web.jhp6.web.service.CommunityQueryService;
...
import static web.jhp6.web.web.rest.TestUtil.createFormattingConversionService;
...
@SpringBootTest(classes = JhipsterPress06App.class)
public class CommunityResourceIntTest {
    ...
    @Test
    @Transactional
    public void getAllCommunitiesByBlockinguserIsEqualToSomething() throws Exception {
        // Initialize the database
        Blockeduser blockinguser = BlockeduserResourceIntTest.createEntity(em);
        em.persist(blockinguser);
        em.flush();
        community.addBlockinguser(blockinguser);
        communityRepository.saveAndFlush(community);
        Long blockinguserId = blockinguser.getId();

        // Get all the communityList where blockinguser equals to blockinguserId
        defaultCommunityShouldBeFound("blockinguserId.equals=" + blockinguserId);

        // Get all the communityList where blockinguser equals to blockinguserId + 1
        defaultCommunityShouldNotBeFound("blockinguserId.equals=" + (blockinguserId + 1));
    }


    @Test
    @Transactional
    public void getAllCommunitiesByBlockeduserIsEqualToSomething() throws Exception {
        // Initialize the database
        Blockeduser blockeduser = BlockeduserResourceIntTest.createEntity(em);
        em.persist(blockeduser);
        em.flush();
        community.addBlockeduser(blockeduser);
        communityRepository.saveAndFlush(community);
        Long blockeduserId = blockeduser.getId();

        // Get all the communityList where blockeduser equals to blockeduserId
        defaultCommunityShouldBeFound("blockeduserId.equals=" + blockeduserId);

        // Get all the communityList where blockeduser equals to blockeduserId + 1
        defaultCommunityShouldNotBeFound("blockeduserId.equals=" + (blockeduserId + 1));
    }
